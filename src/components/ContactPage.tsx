...
import { getSupportEmail } from "../utils/contact";

interface ContactPageProps {
  setActiveTabInApp?: (tab: string) => void;
}

export default function ContactPage({ setActiveTabInApp }: ContactPageProps) {
  return (
    <div className="min-h-[60vh] bg-slate-900 text-slate-100 flex flex-col items-center justify-center p-6 border-t border-slate-800">
      <div className="max-w-md text-center space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-bold tracking-tight text-white mb-2 font-sans">Contact Us</h1>
          <p className="text-sm text-slate-400 leading-relaxed font-light font-sans">
            Need to reach our team immediately? Our operations are dynamic. For booking disputes or group inquiry reservations, please send an mail to <span className="text-indigo-400 font-medium">{getSupportEmail()}</span>.
          </p>
        </div>
        <div>
          <button
